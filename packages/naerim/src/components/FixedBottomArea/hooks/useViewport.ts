...
export default function useViewport() {
  ...
  const createPlaceholderElement = useMemo(() => {
    ...
    hiddenDiv.style.cssText = 'position: fixed; top: 0; bottom: 0; left: 0; right: 0; visibility: hidden;';
    return hiddenDiv;
  }, []);
  ...
  useEffect(() => {
    ...
    const handleViewportChange = () => {
      ...
      const calculatedOffset =
        viewport.height - createPlaceholderElement.getBoundingClientRect().height + viewport.offsetTop;
